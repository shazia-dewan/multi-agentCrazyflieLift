...
def train_PPO(
    ...
    if save_model:
        agent.save(MAPPO_MODEL_SAVE_PATH)
...
# Rendering PPO
####################
def render_PPO(agent: MAPPOAgent, env: CrazyflieEnv, seed: int | None = 42):
    """
...
if __name__ == "__main__":
    ...
    env_fns = [make_env(MULTI_SCENE_PATH, rank=i, seed=42, num_drones=2) for i in range(args.num_envs)]
    if args.num_envs == 1:
    ...
    if not args.load_model:
        ...
        if args.load_model is True:
            model_path = MAPPO_MODEL_SAVE_PATH 
        else:
    ...
    render_env = CrazyflieEnv(
        xml_path=MULTI_SCENE_PATH,
        num_drones=2,
        target_pos=np.array([0.5, 0.5, 2.0], dtype=np.float32),
        max_steps=1500,
